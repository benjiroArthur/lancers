<template>
    <div class="row">
        <div class="col-md-12">
            <h6 class="text-uppercase font-weight-bold">Users (Freelancers)</h6>
            <div class="card-body table table-responsive table-borderless p-0">
                <bootstrap-table :data="lancers" :options="myOptions" :columns="myColumns" sticky-header responsive borderless/>
            </div>
        </div>
...
<script>
    ...
    export default {
        ...
        methods:{
            getLancers(){
                axios.get('/data/admin/getLancers')
                    .then((response)=>{
                        this.lancers = response.data;
                    })
                    .catch()
            },
        },
